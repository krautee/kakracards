...
- First line, centre: an abbreviation (often `li`), then the ring position (e.g. `VÜP`) and the ring number (e.g. `UA6521`). If the header line does not repeat them, the header `ring_position` and `ring_number` are those of the first observation row.
...
RECOVERY rows (the bird was found or reported elsewhere):
- A row whose status mark is a bare `!` (an exclamation mark alone, not `!o`) is a recovery, not an observation. It usually has a date, a place (often abroad, e.g. `Amsterdam NL`, `Tollensesee`), a finder's name and remarks instead of `K'YY-NNN`.
- Put such rows in `recovery` with `recovery_status` = `!`, `recovery_date`, `recovery_location`, `recovery_person`, `recovery_notes`; do not also list them in `content`. Recovery text may also appear on the back of the card.

BACK (lower half):
- The bird ID is the prominent 4-digit number in the centre.
- Any other text on the back is overflow: continue observation or recovery rows from it.
...
- If a field is not written on the card, output an empty string (except the inheritance rules above: ring number/position carry down the rows and up into the header). Never put observation text into `ring_number`, or a `P`-code into `ringing_nest` unless it is clearly the nest.

...
{
    $columns = ['ring_position', 'ring_number', 'obs_status', 'obs_year', 'obs_nest', 'obs_notes', 'decoding_status'];
    $headers = ['Position', 'Number', 'Status', 'Year', 'Nest', 'Notes', 'Decoding'];
    ...
    if (empty($rows)) {
        $html .= '<tr><td colspan="' . (count($columns) + 1) . '" style="border:1px solid #d7dce2;padding:16px;text-align:center;color:#999;font-style:italic;">No content rows</td></tr>';
    } else {
...
function renderRecoveryRowsTable(array $rows): string
{
    ...
    if (empty($rows)) {
        $html .= '<tr><td colspan="' . (count($columns) + 1) . '" style="border:1px solid #d7dce2;padding:16px;text-align:center;color:#999;font-style:italic;">No recovery rows</td></tr>';
    } else {
        foreach ($rows as $i => $row) {
            $html .= '<tr style="' . ($i % 2 ? 'background:#f9fafb;' : '') . '">';
            foreach ($columns as $col) {
...
function buildComparisonData(array $jobs): array
{
    ...
    $contentKeys = ['ring_position', 'ring_number', 'obs_status', 'obs_year', 'obs_nest', 'obs_notes', 'decoding_status'];
